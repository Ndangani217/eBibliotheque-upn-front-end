...
import { DollarSign, Clock, Star, ShieldCheck, Zap, CheckCircle } from 'lucide-react'
...
export default function SubscriptionCards() {
    ...
    const { data: vouchers } = usePayments()
    ...
    // Vérifie si un bon actif existe pour une durée donnée
    const hasVoucherForDuration = (duration: number) =>
        vouchers?.some(
            (v) => v.duration === duration && (v.status === 'en_attente' || v.status === 'payé'), // bon encore actif
        )
        ...
        <section className="mt-10 w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {formulas.map(({ id, duration, price, color, icon: Icon }) => {
                    const alreadyGenerated = hasVoucherForDuration(duration)
                    ...
                    return (
                        ...
                        >
                            {/*Icône discrète si le bon est déjà généré */}
                            {alreadyGenerated && (
